      ...
      <div className="container w-full flex items-center justify-between">
        ...
        <div className="relative" ref={langRef}>
          <motion.div
            ...
            className="flex items-center gap-2.5 h-11 px-4 rounded-full bg-black/40 backdrop-blur-xl border border-white/10 cursor-pointer hover:bg-black/50 transition-all group shadow-lg"
          >
            ...
            <span className="text-white font-bold text-xs tracking-widest uppercase">{currentLang.code}</span>
            <ChevronDown 
              size={12} 
          ...
          <AnimatePresence>
            {langOpen && (
              <motion.div
                ...
                className="absolute top-14 left-0 w-40 bg-black/80 backdrop-blur-2xl rounded-2xl overflow-hidden border border-white/10 p-1.5 z-[101] shadow-2xl"
              >
                {languages.map((lang) => (
                  <button
                    key={lang.code}
        ...
        <motion.div 
          ...
          className="hidden md:flex items-center bg-black/40 backdrop-blur-2xl border border-white/10 p-1.5 rounded-full shadow-2xl"
        >
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  'relative px-6 py-2.5 text-[11px] font-bold tracking-widest uppercase transition-all rounded-full',
        ...
        <div className="flex items-center gap-3">
          ...
          <motion.button 
            ...
            whileTap={{ scale: 0.95 }}
            className="hidden md:flex w-11 h-11 items-center justify-center rounded-full bg-black/40 backdrop-blur-xl border border-white/10 text-white/50 hover:text-white hover:border-white/20 transition-all cursor-pointer relative shadow-lg"
          >
            <Bell size={18} />
            <span className="absolute top-3 right-3 w-2 h-2 bg-primary rounded-full border-2 border-black animate-pulse" />
          </motion.button>
            ...
            <AnimatePresence>
              {profileOpen && (
                <motion.div
                  ...
                  className="absolute top-14 right-0 w-64 bg-black/90 backdrop-blur-2xl rounded-3xl overflow-hidden shadow-2xl border border-white/10 p-5 z-[101]"
                >
                  <div className="flex flex-col items-center text-center">
                    ...
                    <h4 className="text-white font-black text-lg tracking-tight">Salina Gomez</h4>
                    <p className="text-white/40 text-[10px] uppercase tracking-widest font-bold mt-1">Premium Member</p>
                  </div>

                  <div className="mt-6 flex flex-col gap-2">
                    <Link href="/settings" onClick={() => setProfileOpen(false)} className="w-full flex items-center gap-3 p-3 rounded-2xl cursor-pointer bg-white/5 hover:bg-white/10 text-white/60 hover:text-white font-bold transition-all text-xs group">
                      <div className="w-8 h-8 rounded-xl bg-white/5 flex items-center justify-center group-hover:bg-primary/20 transition-colors">
                        <Settings size={14} />
                      </div>
                      {t('navbar.settings')}
                    </Link>
                    <button className="w-full flex items-center gap-3 p-3 rounded-2xl cursor-pointer bg-red-500/10 hover:bg-red-500/20 text-red-400 hover:text-red-300 font-bold transition-all text-xs group">
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="flex justify-between items-center mb-10 pb-6 border-b border-white/10">
              <span className="text-white font-black tracking-widest text-lg">MENU</span>
              <button 
              ...
              {menuItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    onClick={() => setMobileMenuOpen(false)}
                    className={cn(
                      "text-3xl font-black transition-colors w-fit border-b-2 py-1",
